  ...
  MODULE my_vrbls
    ...
    integer, parameter            :: Nseg=8
    double precision              :: SubMSeg(Nseg,4)           ! Magnetization on 4 sublattices
    double precision              :: SubMSeg_Stag(Nseg,4)      ! Staggered Magnetization on 4 Sublattice
    double precision              :: AvgM
    double precision              :: AvgM2
    double precision              :: AvgM4
    double precision              :: AvgM_Stag
    double precision              :: AvgM2_Stag
    double precision              :: AvgM4_Stag
    !----------------------------------------------------------------
  ...
  !subroutine SubM_Eq()
    ...
    !integer :: EndSite,BeginSite, BeginSeg, EndSeg
    !double precision :: BeginTime, EndTime
    !SubMSeg=0.0
    !SubMSeg_Stag=0.0
    !do Vertex = 1, Vol
      ...
      !SubMSeg(i,typ)=SubMSeg(i,typ)+Sz
      !SubMSeg_Stag(i,typ)=SubMSeg_Stag(i,typ)+Sz*get_Sign(Vertex)
    !enddo
    !SubMSeg=SubMSeg/Vol*4.0
    !SubMSeg_Stag=SubMSeg_Stag/Vol*4.0
    !AvgM=0.0; AvgM2=0.0; AvgM4=0.0
    !AvgM_Stag=0.0; AvgM2_Stag=0.0; AvgM4_Stag=0.0
    !!do i=1, Nseg
    !!print *, SubMSeg(1,1)
    !i=1
      !!AvgM=AvgM+(abs(SubMSeg(i,1)))/2.0/Nseg
      !!AvgM2=AvgM2+((SubMSeg(i,1)**2))/4.0/Nseg
      !!AvgM4=AvgM4+((SubMSeg(i,1)**4))/16.0/Nseg
      !AvgM=AvgM+(abs(SubMSeg(i,1)))/2.0
      !AvgM2=AvgM2+((SubMSeg(i,1)**2))/4.0
      !AvgM4=AvgM4+((SubMSeg(i,1)**4))/16.0
    !!enddo
  ...
  subroutine SubM_Eq()
    ...
    do Vertex = 1, Vol
      ...
      do while(BeginSite/=2)
          ...
          EndSeg=EndTime/Beta*Nseg+1
          !if(EndSite==2) then
            !if(EndSeg/=Nseg+1) then
              !print *, "wrong", EndSeg, Nseg
            !endif
            !EndSeg=EndSeg-1
          !endif
          do i=BeginSeg, EndSeg
            Sz=SegmentState(Vertex,BeginSite)
            SubMSeg(i,typ)=SubMSeg(i,typ)+Sz
            SubMSeg_Stag(i,typ)=SubMSeg_Stag(i,typ)+Sz*get_Sign(Vertex)
          enddo
          BeginSeg=EndSeg+1
    ...
    AvgM_Stag=0.0; AvgM2_Stag=0.0; AvgM4_Stag=0.0
    do i=1, Nseg
      AvgM=AvgM+(sum(abs(SubMSeg(i,:))))/2.0/Nseg/4.0
      AvgM2=AvgM2+(sum(SubMSeg(i,:)**2))/4.0/Nseg/4.0
      AvgM4=AvgM4+(sum(SubMSeg(i,:)**4))/16.0/Nseg/4.0
    enddo
  end subroutine SubM_Eq
